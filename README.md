...
当前已实现 **REQ-003 可运行核心**：任务与步骤管理、试跑确认、顺序执行、SQLite 结果传递、运行控制及可选 AI 编写服务。提供 CLI 和本地 HTTP API；真实 Playwright 插件现已接入；跨平台工作台现已实现，Windows 便携包由 REQ-007 实现。旧 Excel 实现已移除。
...
独立客户端：`uv run --extra gui --extra browser --extra sample taskweave workbench`。
...
本地验证码识别插件：启动命令增加 `--extra captcha`，插件管理启用 captcha，步骤与 Playwright 配合使用。见 [插件指南](plugins/captcha/README.md)。
