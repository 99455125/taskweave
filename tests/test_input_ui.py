            ...
            with output.open('w') as log:
                ...
                try:
                    ...
                    with sync_playwright() as runtime:
                        ...
                        self.assertEqual(first_button.locator('.q-icon').inner_text(),'check_circle')
                        ...
                        page.get_by_label('默认值', exact=True).fill('23')
                        ...
                        expect(page.get_by_text('调试状态：成功',exact=True)).to_be_visible(timeout=15000)
                        self.assertEqual(page.get_by_role('button',name='调试当前步骤',exact=True).evaluate('e=>getComputedStyle(e).backgroundColor'),'rgb(240, 253, 250)')
                        ...
                        actions=[page.get_by_role('button',name=title,exact=True).bounding_box() for title in ['调试当前步骤','从选定步骤调试','结束调试']]
                        ...
                        self.assertEqual(page.get_by_role('button',name='结束调试',exact=True).count(),1)
                        env=page.locator('.q-select').filter(has=page.get_by_text('运行 / 调试环境',exact=True))
                        expect(env).to_contain_text('uat')
                        page.get_by_role('button',name='调试当前步骤',exact=True).click()
                        expect(page.get_by_role('button',name='调试当前步骤',exact=True)).to_be_enabled(timeout=15000)
                        expect(page.get_by_text('调试状态：成功',exact=True)).to_be_visible(timeout=15000)
                        page.get_by_role('button',name='结束调试',exact=True).click()
                        page.get_by_role('button',name='确认结束',exact=True).click()
                        expect(page.get_by_role('button',name='结束调试',exact=True)).to_be_disabled(timeout=10000)
                        page.get_by_role('tab',name='步骤详情',exact=True).click()
                        page.get_by_role('button',name='确认验证并保存',exact=True).click()
                        ...
                        expect(page.get_by_text('运行状态：成功',exact=True)).to_be_visible(timeout=15000)
                        expect(page.get_by_role('button',name='收起详情',exact=True)).to_be_visible()
                        ...
                        expect(result_modal.get_by_role('code').get_by_text('23',exact=True)).to_be_visible()
                        result_modal.get_by_role('tab',name='原始数据',exact=True).click()
                        expect(result_modal.get_by_text('"number": 23',exact=False)).to_be_visible()
