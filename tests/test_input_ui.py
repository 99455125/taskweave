            ...
            with output.open('w') as log:
                ...
                try:
                    ...
                    with sync_playwright() as runtime:
                        ...
                        page.get_by_role('tab',name='输入与依赖',exact=True).click()
                        ...
                        env=modal.locator('.q-select').filter(has=page.get_by_text('运行 / 试跑环境',exact=True))
                        env.click();page.get_by_role('option',name='uat',exact=True).click()
                        expect(modal.get_by_label('org · 必录',exact=True)).to_have_attribute('placeholder','未配置默认值，请录入')
                        expect(modal.get_by_label('org · 必录',exact=True)).to_have_value('')
                        modal.get_by_label('org · 必录',exact=True).fill('uat-org')
                        modal.get_by_label('number · 必录',exact=True).fill('17')
                        modal.get_by_role('button',name='开始试跑',exact=True).click()
