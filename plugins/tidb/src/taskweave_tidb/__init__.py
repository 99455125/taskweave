...
class DatabaseAction:
    def __init__(self, operation):
        ...
        if operation == 'query':
            ...
            required = ['table']
        self.spec = CapabilitySpec('tidb.' + operation, {'query': '参数化只读查询，返回可展示的表格和查询信息', 'describe_table': '读取表字段、类型、默认值与注释', 'test_connection': '测试数据库连接，返回版本与当前数据库'}[operation], {'type': 'object', 'properties': props, 'required': required, 'additionalProperties': False}, {'type': 'object'}, 'READ', timeout_ms=65000, retry_safe=True)
...
class TiDBPlugin:
    ...
    return ctx.result(data={"order_no": inputs["order_no"], "verified": passed, "report": report}, views=[{"title": "订单核对", "renderer": "tidb.verification", "pointer": "/report"}])'''
        instructions = 'TiDB uses environment/ task variables tidb_connection_url or tidb_host/port/database/username/password; task overrides environment. Do not pass or copy credentials into action parameters, code or reports. tidb.query takes sql and params (list); bind all business values with %s, never SQL string interpolation. Only SELECT is allowed, including read-only CTEs; never write data. Returns title, columns(key,label), rows(objects), row_count, truncated, query_info. Decimal values are strings; dates are ISO strings. Never invent table or field names: use supplied schema context, tidb.describe_table or request missing schema. Check truncated before verification; do not imply partial rows are complete. Business matching rules belong in the step. Save report in data and declare tidb.verification or tidb.table views. A mismatch can be saved as verified=false for inspection and later flow decisions; it is not a runtime query error. A SQL/connection error fails the step.'
        return AuthoringContribution(instructions, (example,), tool_ids=('tidb.describe_table',), context_provider_ids=('tidb.schema',), channel_overrides={'web_chat': {'instructions': instructions + ' Web chat cannot access the local database. Use only pasted schema context; request missing schema instead of pretending to query.', 'examples': (example,)}})
